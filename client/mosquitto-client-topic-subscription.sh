...
if [ "$#" -lt 2 ]; then
    echo "Usage: $0 <broker_address> <port> <topic_name> <username> <password>"
...
mosquitto_sub -h "$BROKER_ADDRESS" -p "$PORT" -t "$TOPIC_NAME/$SUBTOPIC_NAME/$TOPIC_VALUE" -r --username "$USERNAME" --password "$PASSWORD"

if [ $? -eq 0 ]; then
    echo "Topic '$TOPIC_NAME' subscribed to on broker '$BROKER_ADDRESS'."
else
    echo "Failed to subscribe to topic '$TOPIC_NAME'."
    exit 1
fi

read -p "Would you like to add another subscription? (y/n): " ADD_SUBSCRIPTION
if [[ "$ADD_SUBSCRIPTION" =~ ^[Yy]$ ]]; then
    read -p "Enter Subtopic Name: " SUBTOPIC_NAME
    ...
    if [ -z "$SUBTOPIC_VALUE" ]; then
        echo "Error: Subtopic value is required."
        exit 1
    fi
    # Check if the subtopic value contains invalid characters
    if ! [[ "$SUBTOPIC_VALUE" =~ ^[a-zA-Z0-9/_-]+$ ]]; then
        echo "Error: Invalid subtopic value format. Use alphanumeric characters, underscores, dashes, and slashes."
        exit 1
    fi
    # Subscribe to  message
    mosquitto_sub -h "$BROKER_ADDRESS" -p "$PORT" -t "$TOPIC_NAME/$SUBTOPIC_NAME/$TOPIC_VALUE" -r --username "$USERNAME" --password "$PASSWORD" -m "$MESSAGE"
    if [ $? -eq 0 ]; then
        echo "Subtopic '$SUBTOPIC_NAME' successfully subscribed to '$BROKER_ADDRESS'."
    else
        echo "Failed to subscribe to '$SUBTOPIC_NAME'."
        exit 1
    fi
fi
